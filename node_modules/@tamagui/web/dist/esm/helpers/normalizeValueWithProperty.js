...
function normalizeValueWithProperty(value, property) {
  if (!isWeb || typeof value == "boolean" || property && property in stylePropsUnitless || property && !(property in stylePropsAllPlusTransforms))
    return value;
  let res = value;
...
function reverseMapClassNameToValue(key, className) {
  ...
  let res;
  return cssVal.startsWith("var(") ? res = cssVal : stylePropsUnitless[key] ? res = +cssVal : cssVal.endsWith("px") ? res = +cssVal.replace("px", "") : res = cssVal, rcache[cssRule] = res, process.env.NODE_ENV === "development" && typeof res == "number" && isNaN(res) && console.log("Tamagui invalid parsed value, NaN:", {
    res,
    cssVal,
    cssRule,
    key,
