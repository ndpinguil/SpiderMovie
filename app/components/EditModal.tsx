...
const EditModal = ({closeEditModal,isCreate, data, header}: { closeEditModal: () => void, isCreate: boolean, data: any[], header: string}) => {
  ...
  const confirmFormUpload = () => {
    if (Platform.OS === "web") {
      if (window.confirm("Seguro de subir los siguientes datos?")) {
      ...
      Alert.alert(
        "Subida de Datos",
        "Esta seguro de subir los cambios realizados?",
        [
          {
            text: "CANCELAR",
